...
o3.monitors <- get.monitors(par=44201,yr1=curr.year-9,yr2=curr.year,all=TRUE)
o3.seasons <- get.seasons(yr1=curr.year-9,yr2=curr.year)
pm.monitors <- get.monitors(par=88101,yr1=curr.year-9,yr2=curr.year,all=TRUE)

## Remove monitors that are NAAQS excluded for the entire 10 years
o3.monitors <- o3.monitors[-c(which(o3.monitors$nonreg_concur == "Y" &
  as.Date(gsub(" ",as.character(Sys.Date()),o3.monitors$nonreg_begin_date)) <= as.Date(paste(curr.year-9,"01-01",sep="-")) &
  as.Date(gsub(" ",as.character(Sys.Date()),o3.monitors$nonreg_end_date)) >= as.Date(paste(curr.year,"12-31",sep="-")))),]
pm.monitors <- pm.monitors[-c(which(pm.monitors$nonreg_concur == "Y" &
  as.Date(gsub(" ",as.character(Sys.Date()),pm.monitors$nonreg_begin_date)) <= as.Date(paste(curr.year-9,"01-01",sep="-")) &
  as.Date(gsub(" ",as.character(Sys.Date()),pm.monitors$nonreg_end_date)) >= as.Date(paste(curr.year,"12-31",sep="-")))),]
...
WHERE ss.parameter_code = '88101'
  ...
  AND ss.sample_day >= TO_DATE('",curr.year-9,"-01-01','YYYY-MM-DD')
...
dt.begin <- paste(curr.year-9,"01-01 00:00:00",sep="-")
