...
```{r label="get_schedules",echo=FALSE,message=FALSE,warning=FALSE}
## Function to get PM2.5 site-level sample schedules for a range of years
get.schedules <- function(yr1,yr2) {
  schedules <- get.aqs.data(paste("SELECT DISTINCT
  ...
  WHERE ss.parameter_code = '88101'
    ...
    AND ss.sample_day >= TO_DATE('",yr1,"-01-01','YYYY-MM-DD')
    AND ss.sample_day <= TO_DATE('",yr2,"-12-31','YYYY-MM-DD')
...
get.monitors <- function(par,yr1,yr2,all=FALSE) {
  ...
  WHERE agencies.agency_code = monitors.monitoring_agency
    ...
    AND methods.mo_id = monitors.mo_id
...
```{r label="update_aqs_data",echo=FALSE,message=FALSE,warning=FALSE}
...
```{r label="update_AQS_data",echo=FALSE,message=FALSE,warning=FALSE}
aqs.list <- list(o3.methods=o3.methods,o3.monitors=o3.monitors,o3.seasons=o3.seasons,pm.monitors=pm.monitors,pm.schedules=pm.schedules,
  o3.concurrences=o3.concurrences,pm.concurrences.hourly=pm.concurrences.hourly,pm.concurrences.daily=pm.concurrences.daily)
library(pins)
board <- pins::board_connect()
pin_write(board,x=aqs.list,name="EE_DVcalc_AQSdata",type="rds",title="AQS data for EE_DVcalc app",
  description=paste("Last updated on",Sys.Date()),versioned=TRUE)
